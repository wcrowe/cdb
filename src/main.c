...
int main(int argc, char *argv[]) {
  ...
  if (create_new) {
    ...
    /* Header is in host order — write it directly */
    lseek(fd, 0, SEEK_SET);
  ...
  /* At this point: fd is open, dbhdr is valid and in host order, file pointer is after header */
  ...
  /* output_file() handles all byte-order conversion when writing */
  if (output_file(fd, dbhdr, employees) == STATUS_ERROR) {
    fprintf(stderr, "Failed to save database\n");
    close(fd);
    free(dbhdr);
